...
const ConceptCard = ({
  ...
  <div
    className={`flex items-center gap-3 bg-slate-800/80 backdrop-blur-sm border border-slate-700/50 rounded-lg px-4 py-2 shadow-lg ${className}`}
  >
    {icon}
...
const LoginScreen: React.FC<LoginScreenProps> = ({ onStart }) => {
  ...
  const [error, setError] = useState<string | null>(
    LOGIN_BLOCKED ? LOGIN_BLOCK_MESSAGE : null
  );
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 overflow-hidden">
      ...
      <div className="relative z-10 grid lg:grid-cols-2 gap-10 items-center max-w-6xl mx-auto flex-grow justify-center">
        <div className="relative h-96 hidden lg:flex items-center justify-center">
          <ConceptCard
            icon={<BrainIcon className="w-7 h-7 text-cyan-400" />}
            name="AI Models"
            className="absolute top-10 left-0 animate-float"
          />
          <ConceptCard
            icon={<CodeBracketSquareIcon className="w-7 h-7 text-green-400" />}
            name="JSON Data"
            className="absolute top-32 right-0 animate-float animation-delay-1000"
          />
          <ConceptCard
            icon={<CubeTransparentIcon className="w-7 h-7 text-yellow-400" />}
            name="Frontend/Backend"
            className="absolute bottom-20 left-10 animate-float animation-delay-2000"
          />
          ...
          <div className="flex flex-col gap-4 w-full">
            ...
            >
              <input
                ...
                className="flex-1 bg-slate-800/80 backdrop-blur-sm border border-slate-700/50 rounded-lg px-4 py-2 shadow-lg"
                type="email"
                placeholder="Email"
                value={email}
              ...
              <div className="flex flex-col gap-4">
                <button
                  disabled={LOGIN_BLOCKED || isLoginLoading}
                ...
                <button
                  ...
                  disabled={LOGIN_BLOCKED || isGoogleSignInLoading}
                  className="bg-gradient-to-r from-gray-400 to-blue-500 hover:from-cyan-500 hover:to-blue-600 text-white font-bold py-2 px-10 rounded-full text-lg transition-all duration-300 transform hover:scale-105 shadow-2xl shadow-cyan-500/20 sm:w-auto flex-shrink-0"
                  type="submit"
                >
      ...
      <footer className="w-full text-center text-xs text-slate-600 py-4 mt-auto flex-shrink-0">
        <p>
          © {new Date().getFullYear()} GreyWaken by greybrain.ai. Awaken your
          potential.
        </p>
      </footer>
    </div>
